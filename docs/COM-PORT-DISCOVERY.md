...
**Refresh Serial Ports** asks pyserial for the currently enumerated
...
On Windows this normally shows `COM` names. On macOS and Linux it shows device
paths such as `/dev/cu.usbserial-...` or `/dev/ttyUSB0`.

This scan does not open the ports or send commands.

## Find Compatible Instrument

**Find Compatible Instrument** explicitly probes each serial port using the
...
drop-down. Failed ports remain available through **Refresh Serial Ports**.
